...
def semi_opt(nu_data, mu_data, px, loss_fn=None):
    ...
    X_source = mu_data.view(n_source, 784)
    Y_target = nu_data.view(n_target, 784)
    # sample from params to get X_source
    # [Nu, Nv]
    # c = torch.norm(X_source[:, None]- Y_target[None, :], dim=-1)
    from IPython import embed; embed()
    c = perceptual_loss(loss_fn, X_source[:, None], Y_target[None, :])

    # TODO: implement the loss for 2 images: ? likelihood?
    # Pre-trained resnet to compute?

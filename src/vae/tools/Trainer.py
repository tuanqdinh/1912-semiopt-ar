    ...
    def train(self, n_epochs, save_step=5):
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in enumerate(self.loader, 0):
                ...
                x = out_data.permute(0, 2, 1).expand(-1, -1, 3)
                y = in_data.permute(0, 2, 1).expand(-1, -1, 3)
                # dist1, dist2, _, _ = self.loss_fn(x, y)
            ...
            if epoch % save_step == 0:
                print("Epoch {}/{} - loss: {}".format(epoch, n_epochs,
                        loss.cpu().data.numpy()))
                ...
                try:
                    self.model.save_results(results_dir, out_data, name='sample')
                    self.model.save_results(results_dir, in_data, name='real')
...
class VAETrainer(object):
    ...
    def train(self, n_epochs, save_step=10):
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in enumerate(self.loader, 0):
                ...
                out_data = self.model(in_data)
                x = out_data.permute(0, 2, 1)
                y = in_data.permute(0, 2, 1)
                # dist1, dist2, _, _ = self.loss_fn(x, y)
                # loss = torch.mean(dist1) + torch.mean(dist2)
                ds = self.loss_fn(x, y)
                loss = torch.mean(ds)
            ...
            if epoch % save_step == 0:
                ...
                print("Epoch {}/{} - Total loss: {} | Rec Loss: {} | KLD: {}".format(
                        epoch, n_epochs,
                        loss.cpu().data.numpy() + reg.cpu().data.numpy(),
                        loss.cpu().data.numpy(),
                        reg.cpu().data.numpy()))
                ...
                try:
                    self.model.save_results(results_dir, in_data, name='real')
                    self.model.save_results(results_dir, out_data, name='out')
                    self.model.save_results(samples_dir, self.model.sample(), name='sample')
                except NotImplementedError:
                    print("Intermediate results not saved.")

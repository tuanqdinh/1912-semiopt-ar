...
from vae.models.AutoEncoder import PointCloudVAE
...
netV = PointCloudVAE(obs[1]*obs[2], obs[0], name=args.model_name, enc_size=args.embed_size,
...
for epoch in range(args.num_epochs):
	...
	for step, imgs in enumerate(dataloader):
		...
		vae_data = rescaling_inv(real_data).view(args.batch_size, 3, 1024)
		(latent_batch, recon_batch), mu, logvar = netV(vae_data, latent=True)
		x = recon_batch.permute(0, 2, 1)
		y = vae_data.permute(0, 2, 1)
		ds = loss_fn(x, y)
		loss = torch.mean(ds)
		KLD = kld(mu, logvar)
		...
		if 0 == step % args.log_step:
			...
			print('Mean: {:.4f} Cov-loss: {:.4f}'.format(loss.data.cpu(), KLD.data.cpu()))
			start_time = time.time()
		# sampling
	...
	with torch.no_grad():
		n = 1
		latents, vae_samples = netV.sample(True)
		latents = latents[:n]

...
class PointNetfeat(nn.Module):
    def __init__(self, num_points = 1024, global_feat = True, trans = True):
    ...
    def forward(self, x):
        ...
        if self.trans:
            if self.global_feat:
                ...
                x = x.view(-1, 512, 1).repeat(1, 1, self.num_points)
...
class PointNetCls(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.feat = PointNetfeat(input_dim, global_feat=True)
...
class PointGen(nn.Module):
    ...
    def forward(self, x):
        out = self.model(x)
        out = out.view(x.shape[0], 3, self.num_points)
        return out
